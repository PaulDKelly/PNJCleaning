<div id="edit-site-modal-{{ $site->id }}"
    ...
    <div class="bg-white rounded-xl shadow-xl w-full max-w-md p-6 transform transition-all">
        <div class="flex justify-between items-center mb-6">
            <h3 class="text-lg font-bold text-slate-800">Edit Site: {{ $site->site_name }}</h3>
            <button onclick="document.getElementById('edit-site-modal-{{ $site->id }}').remove()"
        ...
        <form action="{{ route('admin.sites.update', $site->id) }}" method="POST" class="space-y-4">
            ...
            <div>
                ...
                <select name="client_name" required
                    ...
                    @foreach($clients as $c)
                        <option value="{{ $c->client_name }}" {{ $site->client_name == $c->client_name ? 'selected' : '' }}>
            ...
            <div>
                ...
                <input type="text" name="brand_name" value="{{ $site->brand_name }}"
                    class="w-full rounded-lg border-slate-300 focus:border-blue-500 focus:ring-blue-500 text-sm">
            </div>
            <div>
                <label class="block text-sm font-medium text-slate-700 mb-1">Address (Optional)</label>
                <input type="text" name="address" value="{{ $site->address }}"
                    class="w-full rounded-lg border-slate-300 focus:border-blue-500 focus:ring-blue-500 text-sm">
            </div>
            <div class="flex items-center gap-2 mt-4 bg-slate-50 p-3 rounded-lg border border-slate-200">
                <input type="checkbox" name="archived" id="archive-check-{{ $site->id }}"
                    class="rounded border-slate-300 text-blue-600 focus:ring-blue-500 h-4 w-4" {{ $site->archived ? 'checked' : '' }}>
                <label for="archive-check-{{ $site->id }}" class="text-sm font-medium text-slate-700">Archive this
                    site</label>
            </div>

            <div class="pt-4 flex justify-end gap-3">
                <button type="button" onclick="document.getElementById('edit-site-modal-{{ $site->id }}').remove()"
